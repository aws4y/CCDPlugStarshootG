...
enum ReturnStatus
{
	...
	RS_GuiderConflict = -9,		// Dual chip autoguider fighting with main camera shutter
	RS_FilterCFGMissing = -10,	// Cannot find configuration file for filter wheel
	RS_SelfGuideConflict = -11, // A camera with the self-guiding capability (SXAO/AO7) is busy and
	// cannnot execute the current command (for instance, it's calibrating).
	RS_CameraFull = -12,		// Camera cannot take any more images without operator intervention
	RS_Custom = -13,            // Camera will use the advanced plugin interface to report an error
...
enum CoolerState
{
	...
	COOL_DECREASING,		// Cooler temperature is going down  NEW 2000/02/04
	COOL_BROWNOUT,			// Cooler brownout condition  NEW 2001/09/06
	COOL_DISPLAYONLY,		// Can display CCD temperature but no control
};
...
const int MAX_PTITLE = 21;	// Maximum 20 characters in parameter titles, plus NULL char
const int MAX_NSTR = 15;	// Maximum 14 characters in camera model name, plus NULL char
const int MAX_FILTERS = 64;	// Maximum 64 filter positions (large, to hold DSLR ISOs) // Updated 2010/05/14 - Be sure to update version numbers of any DLL referencing this value or recompiled versions might not be installed in an update
const int MAX_COPYRIGHT = 255; // Copyright notice string
...
public:
	...
	// GetCameraStats is called on initialization to get information about the camera's capabilities
	// NOTE:  If XSize is zero, then YSize is a pointer to an ExtendedStatus structure
	virtual void GetCameraStats(CameraInfo& info) = 0;
	...
	// ActivateRelay turns on an autoguider relay output.
	// Both axes may be simultaneously activated.  It is the responsibility of 
	// the plugin to time the activation of each axis independently of the other.
	virtual int ActivateRelay(
		int X,									// Activate X axis for T = abs ( X/100 ) seconds, direction based on sign
		int Y									// Activate Y axis for T = abs ( X/100 ) seconds, direction based on sign
	) = 0;
