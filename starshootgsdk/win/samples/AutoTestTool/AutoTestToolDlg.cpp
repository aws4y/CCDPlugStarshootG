...
LRESULT CAutoTestToolDlg::OnPreviewResChanged(WPARAM wp, LPARAM lp)
{
	...
	HRESULT hr = Starshootg_get_eSize(g_hCam, &nResolutionIndex);
	if (FAILED(hr))
		return -1;

	if (nResolutionIndex != nSel)
	{
		hr = Starshootg_Stop(g_hCam);
		if (FAILED(hr))
			return -1;
		Starshootg_put_eSize(g_hCam, nSel);
...
{
	g_cameraCnt = Starshootg_EnumV2(g_cameras);
...
void CAutoTestToolDlg::OnBnClickedButtonStart()
{
	...
	if (0 == startBtnText.Compare(_T("Open")))
	{
		...
		g_hCam = Starshootg_Open(m_cameraID);
		if (nullptr == g_hCam)
			return;
...
void CAutoTestToolDlg::StartCamera()
{
	...
	Starshootg_get_Size(g_hCam, &width, &height);
	...
	Starshootg_StartPullModeWithWndMsg(g_hCam, m_hWnd, MSG_CAMEVENT);
}
...
void CAutoTestToolDlg::OnEventImage()
{
	...
	HRESULT hr = Starshootg_PullImageV2(g_hCam, m_pImageData, 24, &info);
	if (SUCCEEDED(hr))
	{
		if (g_bROITesting && g_bROITest_StartSnap)
		{
			unsigned offsetX = 0, offsetY = 0,  width = 0, height = 0;
			Starshootg_get_Roi(g_hCam, &offsetX, &offsetY, &width, &height);
		...
		else if (g_bImageShoot)
		{
			...
			Starshootg_get_Size(g_hCam, &resWidth, &resHeight);
			CString str;
			SYSTEMTIME tm;
			GetLocalTime(&tm);
...
{
	StarshootgFrameInfoV2 info = { 0 };
	HRESULT hr = Starshootg_PullStillImageV2(g_hCam, nullptr, 24, &info);
	if (SUCCEEDED(hr))
	{
		void* pData = malloc(TDIBWIDTHBYTES(info.width * 24) * info.height);
		hr = Starshootg_PullStillImage(g_hCam, pData, 24, nullptr, nullptr);
		if (SUCCEEDED(hr))
		{
			BITMAPINFOHEADER header = { 0 };
			header.biSize = sizeof(header);
...
void CAutoTestToolDlg::OnBnClickedButton1()
{
	if (g_cameraCnt <= 0)
		return;
	Starshootg_Replug(g_cameras[m_cameraList.GetCurSel()].id);
}
