...
LRESULT CExposureGainPropertyPage::OnAutoExposure(WPARAM wp, LPARAM lp)
{
	...
	{
		unsigned time = 0;
		Starshootg_get_ExpoTime(g_hCam, &time);
	...
	if (GetDlgItem(IDC_SLIDER_GAIN))
	{
		...
		Starshootg_get_ExpoAGain(g_hCam, &gain);
		SetGainValue(gain);
	}
...
{
	int bAutoExp = 0;
	Starshootg_get_AutoExpoEnable(g_hCam, &bAutoExp);
	...
	((CSliderCtrl*)GetDlgItem(IDC_SLIDER_TARGET))->SetRange(STARSHOOTG_AETARGET_MIN, STARSHOOTG_AETARGET_MAX);
	unsigned short target = 0;
	Starshootg_get_AutoExpoTarget(g_hCam, &target);
	SetTargetValue(target);

	unsigned timeMin = 0, timeMax = 0, timeDef = 0, timeVal = 0;
	Starshootg_get_ExpTimeRange(g_hCam, &timeMin, &timeMax, &timeDef);
	((CSliderCtrl*)GetDlgItem(IDC_SLIDER_EXPOSURE))->SetRange(timeMin, timeMax);
	Starshootg_get_ExpoTime(g_hCam, &timeVal);
	SetExpTimeValue(timeVal);

	unsigned short gainMin = 0, gainMax = 0, gainDef = 0, gainVal = 0;
	Starshootg_get_ExpoAGainRange(g_hCam, &gainMin, &gainMax, &gainDef);
	((CSliderCtrl*)GetDlgItem(IDC_SLIDER_GAIN))->SetRange(gainMin, gainMax);
	Starshootg_get_ExpoAGain(g_hCam, &gainVal);
...
{
	Starshootg_put_AutoExpoEnable(g_hCam, ((CButton*)GetDlgItem(IDC_CHECK_AUTO))->GetCheck());
	UpdateSlidersEnable();
}
...
void CExposureGainPropertyPage::OnHScroll(UINT nSBCode, UINT nPos, CScrollBar* pScrollBar)
{
	if (pScrollBar == GetDlgItem(IDC_SLIDER_TARGET))
	{
		...
		Starshootg_get_AutoExpoTarget(g_hCam, &curTarget);
		unsigned short target = ((CSliderCtrl*)GetDlgItem(IDC_SLIDER_TARGET))->GetPos();
		if (target != curTarget)
		{
			Starshootg_put_AutoExpoTarget(g_hCam, target);
	...
	{
		unsigned curTime = 0;
		Starshootg_get_ExpoTime(g_hCam, &curTime);
		unsigned time = ((CSliderCtrl*)GetDlgItem(IDC_SLIDER_EXPOSURE))->GetPos();
		if (time != curTime)
		{
			Starshootg_put_ExpoTime(g_hCam, time);
			CString str;
	...
	else if (pScrollBar == GetDlgItem(IDC_SLIDER_GAIN))
	{
		...
		Starshootg_get_ExpoAGain(g_hCam, &curGain);
		unsigned short gain = ((CSliderCtrl*)GetDlgItem(IDC_SLIDER_GAIN))->GetPos();
		if (gain != curGain)
		{
			Starshootg_put_ExpoAGain(g_hCam, gain);
