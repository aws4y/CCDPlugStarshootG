...
void CPreviewSnapPropertyPage::UpdateSnapRes()
{
	if (g_hCam)
	{
		int resCnt = Starshootg_get_StillResolutionNumber(g_hCam);
		...
		else
		{
			...
			{
				Starshootg_get_StillResolution(g_hCam, i, &width, &height);
				CString str;
				str.Format(_T("%d x %d"), width, height);
				resList->AddString(str);
			}
...
BOOL CPreviewSnapPropertyPage::OnInitDialog()
{
	...
	if (g_hCam)
	{
		int resCnt = Starshootg_get_ResolutionNumber(g_hCam);
		CComboBox* previewResList = (CComboBox*)GetDlgItem(IDC_COMBO_PREVIEW);
		int width = 0, height = 0;
		for (int i = 0; i < resCnt; ++i)
		{
			Starshootg_get_Resolution(g_hCam, i, &width, &height);
			CString str;
			str.Format(_T("%d x %d"), width, height);
			previewResList->AddString(str);
		}
		unsigned resIndex = 0;
		Starshootg_get_eSize(g_hCam, &resIndex);
...
void CPreviewSnapPropertyPage::OnBnClickedButtonSnap()
{
	...
	{
		unsigned res = 0;
		Starshootg_get_eSize(g_hCam, &res);
		Starshootg_Snap(g_hCam, res);
	}
	else
	{
		CComboBox* resList = (CComboBox*)GetDlgItem(IDC_COMBO_SNAP);
		Starshootg_Snap(g_hCam, resList->GetCurSel());
	}
}
