...
public:
	...
	CExposureTimeDlg(HStarshootg hCam)
	: m_hCam(hCam)
	{
	}
private:
	LRESULT OnInitDialog(UINT /*uMsg*/, WPARAM /*wParam*/, LPARAM /*lParam*/, BOOL& /*bHandled*/)
	{
		...
		if (SUCCEEDED(Starshootg_get_ExpTimeRange(m_hCam, &nMin, &nMax, &nDef)))
		{
			CTrackBarCtrl ctrl(GetDlgItem(IDC_SLIDER1));
			ctrl.SetRangeMin(nMin);
			ctrl.SetRangeMax(nMax);

			if (SUCCEEDED(Starshootg_get_ExpoTime(m_hCam, &nTime)))
				ctrl.SetPos(nTime);
		}
	...
	{
		CTrackBarCtrl ctrl(GetDlgItem(IDC_SLIDER1));
		Starshootg_put_ExpoTime(m_hCam, ctrl.GetPos());
...
private:
	LRESULT OnInitDialog(UINT /*uMsg*/, WPARAM /*wParam*/, LPARAM /*lParam*/, BOOL& /*bHandled*/)
	{
		...
		CTrackBarCtrl ctrl(GetDlgItem(IDC_SLIDER1));
		ctrl.SetRangeMin(0);
		ctrl.SetRangeMax(Starshootg_get_MaxSpeed(m_hCam));

		unsigned short nSpeed = 0;
		if (SUCCEEDED(Starshootg_get_Speed(m_hCam, &nSpeed)))
			ctrl.SetPos(nSpeed);
	...
	{
		CTrackBarCtrl ctrl(GetDlgItem(IDC_SLIDER1));
		Starshootg_put_Speed(m_hCam, ctrl.GetPos());
...
private:
	...
	LRESULT OnOK(WORD /*wNotifyCode*/, WORD wID, HWND /*hWndCtl*/, BOOL& /*bHandled*/)
	{
		...
		if (bTran1 && bTran2)
			Starshootg_put_MaxAutoExpoTimeAGain(m_hCam, nTime, nGain);
		EndDialog(wID);
		return 0;
	}
...
private:
	...
	LRESULT OnButton1(WORD /*wNotifyCode*/, WORD /*wID*/, HWND /*hWndCtl*/, BOOL& /*bHandled*/)
	{
		...
		Starshootg_put_LEDState(m_hCam, nIndex, 1, 0);
		return 0;
	}
	...
	LRESULT OnButton2(WORD /*wNotifyCode*/, WORD /*wID*/, HWND /*hWndCtl*/, BOOL& /*bHandled*/)
	{
		...
		Starshootg_put_LEDState(m_hCam, nIndex, 2, nPeriod);
		return 0;
	}
	...
	{
		const UINT nIndex = GetDlgItemInt(IDC_EDIT1);
		Starshootg_put_LEDState(m_hCam, nIndex, 0, 0);
		return 0;
	}
...
{
	const StarshootgDeviceV2&	m_inst;
	HStarshootg				m_hCam;
...
public:
	...
	CPixelFormatDlg(const StarshootgDeviceV2& tdev, HStarshootg hCam)
...
private:
	LRESULT OnInitDialog(UINT /*uMsg*/, WPARAM /*wParam*/, LPARAM /*lParam*/, BOOL& /*bHandled*/)
	{
		...
		if (0 == (m_inst.model->flag & (STARSHOOTG_FLAG_RAW8 | STARSHOOTG_FLAG_GMCY8)))
			GetDlgItem(IDC_RADIO1).EnableWindow(FALSE);
		
		if (m_inst.model->flag & (STARSHOOTG_FLAG_RAW10 | STARSHOOTG_FLAG_RAW12 | STARSHOOTG_FLAG_RAW14 | STARSHOOTG_FLAG_RAW16 | STARSHOOTG_FLAG_GMCY12))
		{
			wchar_t str[16];
			unsigned bits = 16;
			if (m_inst.model->flag & STARSHOOTG_FLAG_RAW10)
				bits = 10;
			else if (m_inst.model->flag & (STARSHOOTG_FLAG_RAW12 | STARSHOOTG_FLAG_GMCY12))
				bits = 12;
			else if (m_inst.model->flag & STARSHOOTG_FLAG_RAW14)
				bits = 14;
		...
		if (0 == (m_inst.model->flag & (STARSHOOTG_FLAG_VUYY | STARSHOOTG_FLAG_UYVY)))
			GetDlgItem(IDC_RADIO3).EnableWindow(FALSE);

		int val = 0;
		Starshootg_get_Option(m_hCam, STARSHOOTG_OPTION_PIXEL_FORMAT, &val);
		if ((STARSHOOTG_PIXELFORMAT_RAW8 == val) || (STARSHOOTG_PIXELFORMAT_GMCY8 == val))
			CheckDlgButton(IDC_RADIO1, 1);
		else if ((STARSHOOTG_PIXELFORMAT_RAW10 == val) || (STARSHOOTG_PIXELFORMAT_RAW12 == val) || (STARSHOOTG_PIXELFORMAT_RAW14 == val) || (STARSHOOTG_PIXELFORMAT_RAW16 == val) || (STARSHOOTG_PIXELFORMAT_GMCY12 == val))
			CheckDlgButton(IDC_RADIO2, 1);
		else if ((STARSHOOTG_PIXELFORMAT_VUYY == val) || (STARSHOOTG_PIXELFORMAT_UYVY == val))
	...
	{
		if (m_inst.model->flag & STARSHOOTG_FLAG_RAW8)
			Starshootg_put_Option(m_hCam, STARSHOOTG_OPTION_PIXEL_FORMAT, STARSHOOTG_PIXELFORMAT_RAW8);
		else if (m_inst.model->flag & STARSHOOTG_FLAG_GMCY8)
			Starshootg_put_Option(m_hCam, STARSHOOTG_OPTION_PIXEL_FORMAT, STARSHOOTG_PIXELFORMAT_GMCY8);
		return 0;
	}
	...
	{
		if (m_inst.model->flag & STARSHOOTG_FLAG_RAW10)
			Starshootg_put_Option(m_hCam, STARSHOOTG_OPTION_PIXEL_FORMAT, STARSHOOTG_PIXELFORMAT_RAW10);
		else if (m_inst.model->flag & STARSHOOTG_FLAG_RAW12)
			Starshootg_put_Option(m_hCam, STARSHOOTG_OPTION_PIXEL_FORMAT, STARSHOOTG_PIXELFORMAT_RAW12);
		else if (m_inst.model->flag & STARSHOOTG_FLAG_RAW14)
			Starshootg_put_Option(m_hCam, STARSHOOTG_OPTION_PIXEL_FORMAT, STARSHOOTG_PIXELFORMAT_RAW14);
		else if (m_inst.model->flag & STARSHOOTG_FLAG_RAW16)
			Starshootg_put_Option(m_hCam, STARSHOOTG_OPTION_PIXEL_FORMAT, STARSHOOTG_PIXELFORMAT_RAW16);
		else if (m_inst.model->flag & STARSHOOTG_FLAG_GMCY12)
			Starshootg_put_Option(m_hCam, STARSHOOTG_OPTION_PIXEL_FORMAT, STARSHOOTG_PIXELFORMAT_GMCY12);
		return 0;
	}

	LRESULT OnRadio3(WORD /*wNotifyCode*/, WORD /*wID*/, HWND /*hWndCtl*/, BOOL& /*bHandled*/)
	{
		if (m_inst.model->flag & STARSHOOTG_FLAG_VUYY)
			Starshootg_put_Option(m_hCam, STARSHOOTG_OPTION_PIXEL_FORMAT, STARSHOOTG_PIXELFORMAT_VUYY);
		else if (m_inst.model->flag & STARSHOOTG_FLAG_UYVY)
			Starshootg_put_Option(m_hCam, STARSHOOTG_OPTION_PIXEL_FORMAT, STARSHOOTG_PIXELFORMAT_UYVY);
		return 0;
	}
...
public:
	...
	CTECTargetDlg(HStarshootg hCam)
		: m_hCam(hCam)
	{
	}
private:
	LRESULT OnInitDialog(UINT /*uMsg*/, WPARAM /*wParam*/, LPARAM /*lParam*/, BOOL& /*bHandled*/)
	{
		...
		int val = 0;
		Starshootg_get_Option(m_hCam, STARSHOOTG_OPTION_TECTARGET, &val);
	...
	LRESULT OnOK(WORD /*wNotifyCode*/, WORD wID, HWND /*hWndCtl*/, BOOL& /*bHandled*/)
	{
		...
		Starshootg_put_Option(m_hCam, STARSHOOTG_OPTION_TECTARGET, (int)(d * 10));
	...
	LRESULT OnButton1(WORD /*wNotifyCode*/, WORD /*wID*/, HWND /*hWndCtl*/, BOOL& /*bHandled*/)
	{
		...
		if (GetDlgItemText(IDC_EDIT1, strAddr, _countof(strAddr)) && GetDlgItemText(IDC_EDIT2, strLength, _countof(strLength)))
		{
			...
			if (uLength)
			{
				...
				HRESULT hr = Starshootg_read_EEPROM(m_hCam, uAddr, tmpBuffer, uLength);
				if (FAILED(hr))
	...
	LRESULT OnButton2(WORD /*wNotifyCode*/, WORD /*wID*/, HWND /*hWndCtl*/, BOOL& /*bHandled*/)
	{
		...
		if (GetDlgItemText(IDC_EDIT1, strAddr, _countof(strAddr)) && GetDlgItemText(IDC_EDIT2, strLength, _countof(strLength)) && GetDlgItemText(IDC_EDIT3, strData, _countof(strData)))
		{
			...
			if (uLength)
			{
				...
				const HRESULT hr = Starshootg_write_EEPROM(m_hCam, uAddr, tmpBuffer, uLength);
	...
	LRESULT OnButton3(WORD /*wNotifyCode*/, WORD /*wID*/, HWND /*hWndCtl*/, BOOL& /*bHandled*/)
	{
		...
		if (GetDlgItemText(IDC_EDIT2, strLength, _countof(strLength)))
		{
			...
			if (uLength)
			{
				...
				const HRESULT hr1 = Starshootg_write_EEPROM(m_hCam, 0, tmpWriteBuffer, uLength);
				const HRESULT hr2 = Starshootg_read_EEPROM(m_hCam, 0, tmpReadBuffer, uLength);
...
private:
	...
	LRESULT OnButton1(WORD /*wNotifyCode*/, WORD /*wID*/, HWND /*hWndCtl*/, BOOL& /*bHandled*/)
	{
		...
		if (GetDlgItemText(IDC_EDIT2, strLength, _countof(strLength)))
		{
			...
			{
				unsigned char* tmpBuffer = (unsigned char*)alloca(uLength);
				HRESULT hr = Starshootg_read_UART(m_hCam, tmpBuffer, uLength);
				if (FAILED(hr))
	...
	LRESULT OnButton2(WORD /*wNotifyCode*/, WORD /*wID*/, HWND /*hWndCtl*/, BOOL& /*bHandled*/)
	{
		...
		if (GetDlgItemText(IDC_EDIT2, strLength, _countof(strLength)) && GetDlgItemText(IDC_EDIT3, strData, _countof(strData)))
		{
			...
			if (uLength)
			{
				...
				HRESULT hr = Starshootg_write_UART(m_hCam, tmpBuffer, uLength);
				wchar_t strMessage[256];
...
{
	HStarshootg				m_hCam;
	const StarshootgDeviceV2&	m_tdev;
...
public:
	...
	CIocontrolDlg(HStarshootg hCam, const StarshootgDeviceV2& tdev)
...
private:
	...
	{
		Starshootg_IoControl(m_hCam, 0, STARSHOOTG_IOCONTROLTYPE_SET_RESETCOUNTER, 0, NULL);
		return 0;
	}
	...
	LRESULT OnOK(WORD /*wNotifyCode*/, WORD wID, HWND /*hWndCtl*/, BOOL& /*bHandled*/)
	{
		...
		}
		{
			CComboBox box(GetDlgItem(IDC_GPIODIR));
			if (0 == box.GetCurSel())
				Starshootg_IoControl(m_hCam, index, STARSHOOTG_IOCONTROLTYPE_SET_GPIODIR, 0x00, NULL);
			else
				Starshootg_IoControl(m_hCam, index, STARSHOOTG_IOCONTROLTYPE_SET_GPIODIR, 0x01, NULL);
		}
		{
			CComboBox box(GetDlgItem(IDC_OUTPUTINVERTER));
			Starshootg_IoControl(m_hCam, index, STARSHOOTG_IOCONTROLTYPE_SET_OUTPUTINVERTER, box.GetCurSel(), NULL);
		}
		{
			CComboBox box(GetDlgItem(IDC_INPUTACTIVATION));
			Starshootg_IoControl(m_hCam, index, STARSHOOTG_IOCONTROLTYPE_SET_INPUTACTIVATION, box.GetCurSel(), NULL);
		}
		{
			CString str;
			...
			if (!str.IsEmpty())
			{
				...
				Starshootg_IoControl(m_hCam, 0, STARSHOOTG_IOCONTROLTYPE_SET_STROBEDURATION, val, NULL);
			}
		}
		{
			CString str;
			...
			{
				const int val = _ttoi((LPCTSTR)str);
				Starshootg_IoControl(m_hCam, index, STARSHOOTG_IOCONTROLTYPE_SET_DEBOUNCERTIME, val, NULL);
			}
		}
		{
			CString str;
			...
			{
				const int val = _ttoi((LPCTSTR)str);
				Starshootg_IoControl(m_hCam, 0, STARSHOOTG_IOCONTROLTYPE_SET_COUNTERVALUE, val, NULL);
			}
		}
		{
			CComboBox box(GetDlgItem(IDC_OUTPUTMODE));
			Starshootg_IoControl(m_hCam, index, STARSHOOTG_IOCONTROLTYPE_SET_OUTPUTMODE, box.GetCurSel(), NULL);
		}
		{
			CString str;
			...
			{
				const int val = _ttoi((LPCTSTR)str);
				Starshootg_IoControl(m_hCam, 0, STARSHOOTG_IOCONTROLTYPE_SET_USERVALUE, val, NULL);
			}
		}
		{
			CComboBox box(GetDlgItem(IDC_STROBEDELAYMODE));
			Starshootg_IoControl(m_hCam, 0, STARSHOOTG_IOCONTROLTYPE_SET_STROBEDELAYMODE, box.GetCurSel(), NULL);
		}
		{
			CString str;
			...
			{
				const int val = _ttoi((LPCTSTR)str);
				Starshootg_IoControl(m_hCam, 0, STARSHOOTG_IOCONTROLTYPE_SET_STROBEDELAYTIME, val, NULL);
			}
		}
		{
			CString str;
			...
			{
				const int val = _ttoi((LPCTSTR)str);
				Starshootg_IoControl(m_hCam, 0, STARSHOOTG_IOCONTROLTYPE_SET_TRIGGERDELAY, val, NULL);
			}
		}
		{
			CComboBox box(GetDlgItem(IDC_PWMSOURCE));
			Starshootg_IoControl(m_hCam, index, STARSHOOTG_IOCONTROLTYPE_SET_PWMSOURCE, box.GetCurSel(), NULL);
		}
		return 0;
	}
};
...
class CMainFrame : public CFrameWindowImpl<CMainFrame>, public CUpdateUI<CMainFrame>
{
	HStarshootg		m_hCam;
	CMainView		m_view;
	StarshootgDeviceV2	m_dev[STARSHOOTG_MAX];
	unsigned		m_nIndex;
	BOOL			m_bPaused;
...
private:
	LRESULT OnMsgCamEvent(UINT /*uMsg*/, WPARAM wParam, LPARAM /*lParam*/, BOOL& /*bHandled*/)
	{
		switch (wParam)
		{
		case STARSHOOTG_EVENT_ERROR:
		case STARSHOOTG_EVENT_NOFRAMETIMEOUT:
		case STARSHOOTG_EVENT_NOPACKETTIMEOUT:
			OnEventError();
			break;
		case STARSHOOTG_EVENT_DISCONNECTED:
			OnEventDisconnected();
			break;
		case STARSHOOTG_EVENT_EXPOSURE:
			OnEventExpo();
			break;
		case STARSHOOTG_EVENT_TEMPTINT:
			OnEventTemptint();
			break;
		case STARSHOOTG_EVENT_IMAGE:
			OnEventImage();
			if (eTriggerLoop == m_eTriggerType)
				Starshootg_Trigger(m_hCam, 1);
			break;
		case STARSHOOTG_EVENT_STILLIMAGE:
			OnEventSnap();
			break;
		}
		return 0;
	...
	int OnCreate(LPCREATESTRUCT /*lpCreateStruct*/)
	{
		...
		const unsigned cnt = Starshootg_EnumV2(m_dev);
		if (0 == cnt)
	...
	{
		if (m_hCam)
			Starshootg_AwbOnce(m_hCam, NULL, NULL);
	}
	...
	void OnAutoExposure(UINT /*uNotifyCode*/, int /*nID*/, HWND /*wndCtl*/)
	{
		if (m_hCam)
		{
			...
			if (SUCCEEDED(Starshootg_get_AutoExpoEnable(m_hCam, &bAutoExposure)))
			{
				bAutoExposure = !bAutoExposure;
				Starshootg_put_AutoExpoEnable(m_hCam, bAutoExposure);
	...
	void OnVerticalFlip(UINT /*uNotifyCode*/, int /*nID*/, HWND /*wndCtl*/)
	{
		...
		{
			BOOL b = FALSE;
			if (SUCCEEDED(Starshootg_get_VFlip(m_hCam, &b)))
			{
				b = !b;
				Starshootg_put_VFlip(m_hCam, b);
	...
	void OnHorizontalFlip(UINT /*uNotifyCode*/, int /*nID*/, HWND /*wndCtl*/)
	{
		...
		{
			BOOL b = FALSE;
			if (SUCCEEDED(Starshootg_get_HFlip(m_hCam, &b)))
			{
				b = !b;
				Starshootg_put_HFlip(m_hCam, b);
	...
	void OnPause(UINT /*uNotifyCode*/, int /*nID*/, HWND /*wndCtl*/)
	{
		...
		{
			m_bPaused = !m_bPaused;
			Starshootg_Pause(m_hCam, m_bPaused);
	...
	{
		if (m_hCam && (m_dev[m_nIndex].model->flag & STARSHOOTG_FLAG_TEC_ONOFF)) // support set the tec target
		{
	...
	void OnRoi(UINT /*uNotifyCode*/, int /*nID*/, HWND /*wndCtl*/)
	{
		...
		{
			CRoiDlg dlg;
			Starshootg_get_Roi(m_hCam, &dlg.xOffset_, &dlg.yOffset_, &dlg.xWidth_, &dlg.yHeight_);
			if (IDOK == dlg.DoModal())
			{
				if (SUCCEEDED(Starshootg_put_Roi(m_hCam, dlg.xOffset_, dlg.yOffset_, dlg.xWidth_, dlg.yHeight_)))
				{
					Starshootg_get_Roi(m_hCam, NULL, NULL, (unsigned*)&m_header.biWidth, (unsigned*)&m_header.biHeight);
	...
	{
		char ver[16] = { 0 };
		if (SUCCEEDED(Starshootg_get_FwVersion(m_hCam, ver)))
		{
			CA2T a2t(ver);
	...
	{
		char ver[16] = { 0 };
		if (SUCCEEDED(Starshootg_get_HwVersion(m_hCam, ver)))
		{
			CA2T a2t(ver);
	...
	{
		char ver[16] = { 0 };
		if (SUCCEEDED(Starshootg_get_FpgaVersion(m_hCam, ver)))
		{
			CA2T a2t(ver);
	...
	void OnTestPattern(UINT /*uNotifyCode*/, int nID, HWND /*wndCtl*/)
	{
		...
		if (val)
			val = val * 2 + 1;
		Starshootg_put_Option(m_hCam, STARSHOOTG_OPTION_TESTPATTERN, val);
	}

	void OnProductionDate(UINT /*uNotifyCode*/, int /*nID*/, HWND /*wndCtl*/)
	{
		char pdate[10] = { 0 };
		if (SUCCEEDED(Starshootg_get_ProductionDate(m_hCam, pdate)))
		{
			CA2T a2t(pdate);
	...
	{
		char sn[32] = { 0 };
		if (SUCCEEDED(Starshootg_get_SerialNumber(m_hCam, sn)))
		{
			CA2T a2t(sn);
	...
	void OnRawformat(UINT /*uNotifyCode*/, int /*nID*/, HWND /*wndCtl*/)
	{
		...
		if (SUCCEEDED(Starshootg_get_RawFormat(m_hCam, &nFourCC, &bitsperpixel)))
		{
			wchar_t str[257];
	...
	void OnTriggerMode(UINT /*uNotifyCode*/, int /*nID*/, HWND /*wndCtl*/)
	{
		...
		{
			int val = 0;
			Starshootg_get_Option(m_hCam, STARSHOOTG_OPTION_TRIGGER, &val);
			if (val == 0)
				val = (m_dev[m_nIndex].model->flag & STARSHOOTG_FLAG_TRIGGER_EXTERNAL) ? 2 : 1;
			else
				val = 0;
			Starshootg_put_Option(m_hCam, STARSHOOTG_OPTION_TRIGGER, val);
	...
	void OnTriggerNumber(UINT /*uNotifyCode*/, int /*nID*/, HWND /*wndCtl*/)
	{
		...
		if (IDOK == dlg.DoModal())
		{
			...
			if (m_dev[m_nIndex].model->ioctrol > 0)
				Starshootg_IoControl(m_hCam, 0, STARSHOOTG_IOCONTROLTYPE_SET_BURSTCOUNTER, m_nTriggerNumber, NULL);
		}
	}
	...
	void OnTriggerTrigger(UINT /*uNotifyCode*/, int /*nID*/, HWND /*wndCtl*/)
	{
		if (m_hCam)
		{
			...
			{
				m_eTriggerType = eTriggerNumber;
				Starshootg_IoControl(m_hCam, 0, STARSHOOTG_IOCONTROLTYPE_SET_TRIGGERSOURCE, 5, NULL);
				const HRESULT hr = Starshootg_Trigger(m_hCam, m_nTriggerNumber);
				if (E_INVALIDARG == hr)
				{
					if (m_nTriggerNumber > 1)
						AtlMessageBox(m_hWnd, L"STARSHOOTG_FLAG_TRIGGER_SINGLE: only number = 1 supported");
				}
			}
		}
	}
	...
	void OnPreviewResolution(UINT /*uNotifyCode*/, int nID, HWND /*wndCtl*/)
	{
		...
		unsigned eSize = 0;
		if (SUCCEEDED(Starshootg_get_eSize(m_hCam, &eSize)))
		{
			if (eSize != nID - ID_PREVIEW_RESOLUTION0)
			{
				if (SUCCEEDED(Starshootg_Stop(m_hCam)))
				{
					...
					Starshootg_put_eSize(m_hCam, nID - ID_PREVIEW_RESOLUTION0);
					for (unsigned i = 0; i < m_dev[m_nIndex].model->preview; ++i)
						UISetCheck(ID_PREVIEW_RESOLUTION0 + i, (nID - ID_PREVIEW_RESOLUTION0 == i) ? 1 : 0);
					UpdateSnapMenu();
					if (SUCCEEDED(Starshootg_get_Size(m_hCam, (int*)&m_header.biWidth, (int*)&m_header.biHeight)))
					{
						UpdateResolutionText();
						UpdateFrameText(L"");
						UpdateExposureTimeText();
						...
						if (SUCCEEDED(Starshootg_StartPullModeWithWndMsg(m_hCam, m_hWnd, MSG_CAMEVENT)))
						{
	...
	void OnSnapResolution(UINT /*uNotifyCode*/, int nID, HWND /*wndCtl*/)
	{
		...
		if (IDOK == dlg.DoModal())
		{
			...
			if (SUCCEEDED(Starshootg_Snap(m_hCam, nID - ID_SNAP_RESOLUTION0)))
			{
				m_nSnapType = 1;
				m_nSnapSeq = 0;
				UpdateSnapMenu();
	...
	void OnSnapnResolution(UINT /*uNotifyCode*/, int nID, HWND /*wndCtl*/)
	{
		...
		{
			if (SUCCEEDED(Starshootg_SnapN(m_hCam, nID - ID_SNAPN_RESOLUTION0, dlg.m_nNum)))
			{
				m_nSnapType = 2;
	...
	void OnOpenDevice(UINT /*uNotifyCode*/, int nID, HWND /*wndCtl*/)
	{
		...
		m_hCam = Starshootg_Open(m_dev[m_nIndex].id);
		if (m_hCam)
		{
			/* just to demo put roi befor the camera is started */
			if (m_xRoiWidth && m_yRoiHeight)
			{
				Starshootg_put_Roi(m_hCam, m_xRoiOffset, m_yRoiOffset, m_xRoiWidth, m_yRoiHeight);
				Starshootg_get_Roi(m_hCam, NULL, NULL, (unsigned*)&m_header.biWidth, (unsigned*)&m_header.biHeight);
			}
			else
			{
				Starshootg_get_Size(m_hCam, (int*)&m_header.biWidth, (int*)&m_header.biHeight);
			}

			if (m_bTriggerMode)
			{
				if (m_dev->model->flag & STARSHOOTG_FLAG_TRIGGER_EXTERNAL)
					Starshootg_put_Option(m_hCam, STARSHOOTG_OPTION_TRIGGER, 2);
				else if (m_dev->model->flag & STARSHOOTG_FLAG_TRIGGER_EXTERNAL)
					Starshootg_put_Option(m_hCam, STARSHOOTG_OPTION_TRIGGER, 1);
			}
	...
	void OnEventImage()
	{
		...
		HRESULT hr = Starshootg_PullImageV2(m_hCam, m_pData, m_header.biBitCount, &info);
		if (FAILED(hr))
			return;
	...
	void OnEventSnap()
	{
		...
		HRESULT hr = Starshootg_PullStillImage(m_hCam, NULL, 24, (unsigned*)&header.biWidth, (unsigned*)&header.biHeight); //first, peek the width and height
		if (SUCCEEDED(hr))
		{
			...
			if (pSnapData)
			{
				hr = Starshootg_PullStillImage(m_hCam, pSnapData, 24, NULL, NULL);
				if (SUCCEEDED(hr))
				{
					if (2 == m_nSnapType)
					{
	...
	void OnEventTemptint()
	{
		CStatusBarCtrl statusbar(m_hWndStatusBar);
		wchar_t res[128];
		int nTemp = STARSHOOTG_TEMP_DEF, nTint = STARSHOOTG_TINT_DEF;
		Starshootg_get_TempTint(m_hCam, &nTemp, &nTint);
	...
	void OnEventExpo()
	{
		...
		if (SUCCEEDED(Starshootg_get_ExpoTime(m_hCam, &nTime)) && SUCCEEDED(Starshootg_get_ExpoAGain(m_hCam, &AGain)))
		{
	...
	void OnDeviceChanged()
	{
		...
		{
			unsigned eSize = 0;
			Starshootg_get_eSize(m_hCam, &eSize);
			...
			int nTemp = STARSHOOTG_TEMP_DEF, nTint = STARSHOOTG_TINT_DEF;
			if (SUCCEEDED(Starshootg_get_TempTint(m_hCam, &nTemp, &nTint)))
			{
				swprintf(res, L"Temp = %d, Tint = %d", nTemp, nTint);
				statusbar.SetText(2, res);
			}
		...
		UIEnable(ID_TECTARGET, (m_hCam && (m_dev[m_nIndex].model->flag & STARSHOOTG_FLAG_TEC_ONOFF)) ? TRUE : FALSE);
	...
	void UpdateResolutionText()
	{
		...
		if (SUCCEEDED(Starshootg_get_Roi(m_hCam, &xOffset, &yOffset, &nWidth, &nHeight)))
		{
	...
	void UpdateFrameText(const StarshootgFrameInfoV2& info)
	{
		wchar_t str[256];
		if (m_dwLastTick != m_dwStartTick)
		{
			if (info.flag & (STARSHOOTG_FRAMEINFO_FLAG_SEQ | STARSHOOTG_FRAMEINFO_FLAG_TIMESTAMP))
