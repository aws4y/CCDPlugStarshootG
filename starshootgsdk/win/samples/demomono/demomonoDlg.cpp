...
BEGIN_MESSAGE_MAP(CdemomonoDlg, CDialog)
	...
	ON_WM_HSCROLL()
	ON_COMMAND_RANGE(IDM_SNAP_RESOLUTION, IDM_SNAP_RESOLUTION + STARSHOOTG_MAX, &CdemomonoDlg::OnSnapResolution)
END_MESSAGE_MAP()
...
void CdemomonoDlg::OnBnClickedButton1()
{
	...
	int n = (int)Starshootg_get_ResolutionNumber(m_hcam);
	if (n > 0)
	{
		TCHAR txt[128];
		int nWidth, nHeight;
		for (int i = 0; i < n; ++i)
		{
			Starshootg_get_Resolution(m_hcam, i, &nWidth, &nHeight);
			_stprintf(txt, _T("%d * %d"), nWidth, nHeight);
			pCombox->AddString(txt);
		}

		unsigned nCur = 0;
		Starshootg_get_eSize(m_hcam, &nCur);
		pCombox->SetCurSel(nCur);
	}

	GetDlgItem(IDC_CHECK2)->EnableWindow(FALSE);
	m_maxBitDepth = Starshootg_get_MaxBitDepth(m_hcam);
...
void CdemomonoDlg::StartDevice()
{
	...
	Starshootg_StartPullModeWithWndMsg(m_hcam, m_hWnd, MSG_CAMEVENT);

	BOOL bEnableAutoExpo = TRUE;
	Starshootg_get_AutoExpoEnable(m_hcam, &bEnableAutoExpo);
	...
	Starshootg_get_ExpTimeRange(m_hcam, &nMinExpTime, &nMaxExpTime, &nDefExpTime);
...
void CdemomonoDlg::OnCbnSelchangeCombo1()
{
	...
	HRESULT hr = Starshootg_get_eSize(m_hcam, &nResolutionIndex);
	if (FAILED(hr))
		return;

	if (nResolutionIndex != nSel)
	{
		hr = Starshootg_Stop(m_hcam);
		if (FAILED(hr))
			return;

		Starshootg_put_eSize(m_hcam, nSel);
...
{
	unsigned nTime = 0;
	Starshootg_get_ExpoTime(m_hcam, &nTime);
...
{
	StarshootgFrameInfoV2 info;
	/* bits: 24 (RGB24), 32 (RGB32), 8 (Gray) or 16 (Gray). In RAW mode, this parameter is ignored. */
	HRESULT hr = Starshootg_PullImageV2(m_hcam, m_pImageData, m_bBitDepth ? 16 : 8, &info);
	if (SUCCEEDED(hr))
	{
...
{
	Starshootg_Snap(m_hcam, nID - IDM_SNAP_RESOLUTION);
}

void CdemomonoDlg::OnBnClickedButton2()
{
	int n = Starshootg_get_StillResolutionNumber(m_hcam);
	if (n <= 0)
	{
		unsigned e = 0;
		Starshootg_get_eSize(m_hcam, &e);
		Starshootg_Snap(m_hcam, e);
	}
	else
	{
		CPoint pt;
		...
		for (int i = 0; i < n; ++i)
		{
			Starshootg_get_StillResolution(m_hcam, i, &w, &h);
...
{
	if (m_hcam)
		Starshootg_put_AutoExpoEnable(m_hcam, IsDlgButtonChecked(IDC_CHECK1) ? TRUE : FALSE);
...
void CdemomonoDlg::OnHScroll(UINT nSBCode, UINT nPos, CScrollBar* pScrollBar)
{
	if (pScrollBar == GetDlgItem(IDC_SLIDER1))
	{
		...
		Starshootg_put_ExpoTime(m_hcam, nTime * 1000);
	}
